...
class ExpiringTokenAuthentication(TokenAuthentication):
    ...
    def authenticate_credentials(self, key):
        ...
        now=(time_now - datetime.timedelta(minutes=EXPIRE_MINUTES)).replace(tzinfo=pytz.timezone('Asia/Shanghai'))   #转换成上海时区
        ...
        if token:
            # Cache token
            cache.set(key, token.user, EXPIRE_MINUTES * 60*60*12)
